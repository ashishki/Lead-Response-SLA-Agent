...
# Writes a lightweight Orchestrator state snapshot for legacy hook runners.
# This project is Codex-only and does not use a Claude command entrypoint.
#
...
Resume: ask Codex to continue from docs/prompts/ORCHESTRATOR.md.
        Codex re-reads $CODEX_PROMPT automatically.
...
if [ "${SILENT:-0}" != "1" ] \
  ...
  NOTIFY_TEXT="Session ended ${TIMESTAMP}. Active: ${ACTIVE_TASK}. Fix Queue: ${FIX_COUNT}. Resume Codex from docs/prompts/ORCHESTRATOR.md."
