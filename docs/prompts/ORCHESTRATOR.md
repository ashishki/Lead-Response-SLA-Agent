# Lead Response SLA Agent - Workflow Orchestrator
...
<!-- codex exec -s workspace-write is the implementation agent invocation.
     ...
     files under /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent, and execute shell commands (test runner, linter). -->
...
cd /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent && codex exec -s workspace-write "$PROMPT"
```
...
You are the **Orchestrator** for the Lead Response SLA Agent project.
...
Check: does semantic ownership apply? (PLAYBOOK section Capability Signal Patterns)
...
If `docs/ARCHITECTURE.md section Deterministic vs LLM-Owned Subproblems` suggests a softer mismatch, print `DETERMINISM_WARNING` and continue. This is a reviewer signal, not a stop.

If the task clearly increases model class, inference cost envelope, or escalation behavior relative to `docs/ARCHITECTURE.md section Inference / Model Strategy` without an architectural update, print `MODEL_STRATEGY_WARNING` and continue. This is drift, not an automatic stop.
...
```
You are the Strategy Reviewer for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
```
You are the implementation agent for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Run: cd /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent && [YOUR_TEST_COMMAND]
...
- `DONE` + all AC PASS + 0 failures:
  ...
  Signal: [matched pattern] in [file path]
  Suggestion: verify semantic ownership (PLAYBOOK section Capability Signal Patterns) - tag may need correction before this task is archived.
  ```
...
1. Read `docs/CODEX_PROMPT.md section Evaluation State` - was the Last Evaluation entry updated for this task?
...
- Send a focused remediation prompt back to the same implementation agent (not a new full agent):
  ...
  Read docs/IMPLEMENTATION_CONTRACT.md section Retrieval Evaluation Gate (or relevant profile gate).
  Update the evaluation artifact with current results. Compare against baseline.
  Update docs/CODEX_PROMPT.md section Evaluation State section Last Evaluation.
  ...
  - Eval Source: the exact command, script, or manual method used to produce the metrics
    (e.g. "scripts/eval.py against section Evaluation Dataset (10 queries), run YYYY-MM-DD")
...
- Verify `Eval Source` field is present and non-blank in both the evaluation artifact entry and in `docs/CODEX_PROMPT.md section Evaluation State section Last Evaluation`. If absent or blank -> treat as "evaluation NOT performed" (see remediation prompt below).
- Verify `Date` / timestamp is present and non-blank in both locations. If absent -> same treatment.
- **Regression check** - compare the current primary metric against the baseline row in `docs/CODEX_PROMPT.md section Evaluation State section Regression Thresholds`:
  ...
  - If `section Regression Thresholds` field is absent in `docs/CODEX_PROMPT.md` -> use 15 % / 5 % defaults above and add a P2 note to set explicit thresholds.
  - Document any regression (regardless of severity) in the evaluation artifact section Regression Notes with root cause classification: `code-change-induced` or `corpus-change-induced`.
- No regression -> confirm `docs/CODEX_PROMPT.md section Evaluation State section Last Evaluation` is current. Proceed to Step 4.
...
```
You are the Light Reviewer for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Use **Agent tool** (`general-purpose`):
```
You are the META Analyst for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Use **Agent tool** (`general-purpose`):
```
You are the Architecture Reviewer for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Use **Agent tool** (`general-purpose`):
```
You are the Code Reviewer for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Use **Agent tool** (`general-purpose`):
```
You are the Consolidation Agent for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Write to `/tmp/orchestrator_codex_prompt.txt`:
```
You are the Fixer for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Write to `/tmp/orchestrator_codex_prompt.txt`:
```
You are the Fix agent for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
Run: cd /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent && [YOUR_TEST_COMMAND] - must be green.
...
```
You are the Doc Updater for Lead Response SLA Agent.
Project root: /home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent
...
<!-- configured notification channel is optional. It represents any out-of-band notification
...
Notification delivery (adapt or remove for configured notification channel):
```bash
...
Stop when:
- All tasks `[x]` -> generate final completion report (same format as Phase Report, titled "PROJECT COMPLETE") -> send notification -> stop.
...
- API rate limit (429 / "overloaded") -> save checkpoint -> send notification with suggested restart time (current time + 60 min) -> print "RATE_LIMIT_HIT" -> stop cleanly.
  Notification format (adapt to configured notification channel):
  ```
...
This orchestrator is already adapted for Lead Response SLA Agent.

| Constant | Value |
|---|---|
| Project name | `Lead Response SLA Agent` |
| Project root | `/home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent` |
| Implementation command | `codex exec -s workspace-write` |
| Notification channel | Optional; configure outside the repository if needed |
...
2. Can read and write files under `/home/ashishki/Documents/dev/ai-stack/projects/Lead-Response-SLA-Agent`
...
**`configured notification channel` - notification options:**
