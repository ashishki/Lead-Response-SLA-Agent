...
_Copy to `docs/audit/PHASE1_VALIDATOR.md` in your project. Replace `Lead Response SLA Agent`._

```
You are the Phase 1 Validator for Lead Response SLA Agent.
...
- [ ] A1-01  section  System Overview - one paragraph present
- [ ] A1-01a section  Problem Fit and Adoption Reality - present with concrete pain, current workaround, first proof of value, claims not allowed before evidence, and work AI will not replace
- [ ] A1-02  section  Solution Shape - primary shape, governance level, and runtime tier declared with justification
- [ ] A1-03  section  Rejected Lower-Complexity Options - present and non-empty
- [ ] A1-04  section  Minimum Viable Control Surface - present and non-empty
- [ ] A1-05  section  Human Approval Boundaries - present and non-empty
- [ ] A1-06  section  Deterministic vs LLM-Owned Subproblems - present and non-empty
- [ ] A1-07  section  Runtime and Isolation Model - present with at least isolation boundary, runtime mutation boundary, and rollback/recovery
- [ ] A1-08  section  Capability Profiles table - present with all five profiles declared ON or OFF (RAG, Tool-Use, Agentic, Planning, Compliance)
- [ ] A1-09  section  Component Table - at least one row with name, file/directory, responsibility
- [ ] A1-10  section  Data Flow - numbered steps for primary request path
- [ ] A1-11  section  Tech Stack - table present with technology choices and rationale column (not blank)
- [ ] A1-12  section  Security Boundaries - present and non-empty (authentication mechanism described)
- [ ] A1-13  section  External Integrations - present (may be empty table if no integrations; cannot be missing)
- [ ] A1-14  section  File Layout - directory tree present
- [ ] A1-15  section  Runtime Contract - env vars table present (may be empty if no env vars required)
- [ ] A1-16  section  Continuity and Retrieval Model - canonical truth, retrieval convenience, and scoped retrieval rules declared
- [ ] A1-17  section  Non-Goals - explicit list present (at minimum one item, including over-architecture non-goal)
- [ ] A1-18  RAG Profile declared ON or OFF - if ON, section RAG Architecture, section Corpus Description, section Retrieval / Embedding Strategy, section Index Strategy, section Risks all present
- [ ] A1-19  For each active profile declared ON: a justification paragraph is present below the Capability Profiles table
- [ ] A1-20  Compliance Profile declared ON or OFF - if ON, section Applicable Frameworks, section Data Classification, section Audit Log Requirements, section Risks all present

### A2 - docs/spec.md

- [ ] A2-01  section  Overview - present
- [ ] A2-02  section  User Roles - at least one role defined
...
- [ ] A2-05  If RAG Profile = ON: section  Retrieval section present with sources indexed, query types, citation format, insufficient_evidence behavior, and retrieval mode (`text-only` or `multimodal`)
...
- [ ] A4-05  section  Instructions for Codex present (pre-task protocol included)
...
- [ ] A4-11  section  Continuity Pointers present and points to decision log / implementation journal / evidence index usage
...
- [ ] A5-02  section  Universal Rules present (must include: SQL Safety, PII Policy, Credentials/Secrets, CI Gate - at minimum)
- [ ] A5-03  section  Project-Specific Rules present (may be empty if no project-specific rules, but section must exist)
- [ ] A5-04  section  Continuity and Retrieval Rules present with canonical-vs-retrieval boundary and required lookup triggers
- [ ] A5-05  section  Control Surface and Runtime Boundaries present with at least privileged actions, runtime mutation, and auditability; unused rows may be `N/A`
- [ ] A5-06  If Runtime tier = T2 or T3 in ARCHITECTURE.md: conditional rollback / snapshot / drift-management rules are present
- [ ] A5-07  section  Mandatory Pre-Task Protocol present (must include: read contract, run pytest baseline, run ruff, and required continuity lookup when applicable)
- [ ] A5-08  section  Forbidden Actions present (must include at minimum: SQL interpolation, skipping baseline capture, self-closing findings without code verification, deferring CI past Phase 1, unauthorized runtime-tier expansion)
- [ ] A5-09  If RAG Profile = ON: section  RAG Rules present with corpus isolation, schema versioning, max index age, insufficient_evidence requirement, and embedding-strategy declaration rules
- [ ] A5-10  If Tool-Use Profile = ON: section  Tool-Use Rules present
- [ ] A5-11  If Agentic Profile = ON: section  Agentic Rules present
- [ ] A5-12  If Planning Profile = ON: section  Planning Rules present
- [ ] A5-13  If Compliance Profile = ON: section  Compliance Rules present with data field handling, audit log format contract, audit integrity rules, evidence artifact requirements
...
- [ ] B-05  RAG tasks consistency (if RAG = ON): ARCHITECTURE.md declares RAG ON -> tasks.md contains rag:ingestion and rag:query tagged tasks -> IMPLEMENTATION_CONTRACT.md contains section  RAG Rules
...
- [ ] B-06  Tool-Use tasks consistency (if Tool-Use = ON): tasks.md contains tool:schema tagged task -> IMPLEMENTATION_CONTRACT.md contains section  Tool-Use Rules
- [ ] B-07  Agentic tasks consistency (if Agentic = ON): tasks.md contains agent:loop or agent:termination tagged task -> IMPLEMENTATION_CONTRACT.md contains section  Agentic Rules
- [ ] B-08  Planning tasks consistency (if Planning = ON): tasks.md contains plan:schema tagged task -> IMPLEMENTATION_CONTRACT.md contains section  Planning Rules
- [ ] B-08b Compliance tasks consistency (if Compliance = ON): tasks.md contains compliance:control and compliance:audit tagged tasks -> IMPLEMENTATION_CONTRACT.md contains section  Compliance Rules
- [ ] B-08c NFR consistency (if docs/nfr.md exists): SLA Table contains at least one row with a non-empty Target; CODEX_PROMPT.md contains section  NFR Baseline block
- [ ] B-08d Eval artifact consistency: for each profile declared ON in ARCHITECTURE.md, the corresponding evaluation artifact (retrieval_eval.md / tool_eval.md / agent_eval.md / plan_eval.md / compliance_eval.md) is present, initialized, and matches the profile declaration (e.g., compliance_eval.md control rows reference the frameworks declared in ARCHITECTURE.md section Applicable Frameworks)
...
- [ ] B-08f Runtime-tier consistency: ARCHITECTURE.md section Runtime and Isolation Model matches IMPLEMENTATION_CONTRACT.md section Control Surface and Runtime Boundaries at the declared-boundary level
- [ ] B-08g Human approval consistency: ARCHITECTURE.md section Human Approval Boundaries is reflected in IMPLEMENTATION_CONTRACT.md privileged / unsafe action rules
- [ ] B-08h Deterministic ownership consistency: ARCHITECTURE.md section Deterministic vs LLM-Owned Subproblems does not directly contradict task tags or profile declarations
- [ ] B-08i Adoption reality consistency: ARCHITECTURE.md section Problem Fit and Adoption Reality does not make broad AI replacement or autonomy claims unless matching proof metrics, human approval boundaries, and evaluation artifacts are present
...
- [ ] B-10  Tech stack consistency: every technology declared in ARCHITECTURE.md section Tech Stack that requires env vars has those env vars listed in section Runtime Contract
- [ ] B-11  External integrations consistency: every service listed in ARCHITECTURE.md section External Integrations either (a) has env vars in section Runtime Contract, or (b) is documented as not requiring credentials
...
- cannot name the concrete operational pain, current workaround, and first proof
  metric in `docs/ARCHITECTURE.md section Problem Fit and Adoption Reality`
